...
export class SRS extends BaseModule {
    ...
    setupEventListeners() {
        const srsContainer = BrowserUtils.getGlobal().document.querySelector('.srs-container');
        if (!srsContainer) return;

        // Start review button
        const startButton = srsContainer.querySelector('.start-srs');
        if (startButton) {
            BrowserUtils.addEventListener(startButton, 'click', () => this.startReview());
        }

        // Level selector
        const levelSelect = srsContainer.querySelector('.level-select');
        if (levelSelect) {
            BrowserUtils.addEventListener(levelSelect, 'change', (e) => {
